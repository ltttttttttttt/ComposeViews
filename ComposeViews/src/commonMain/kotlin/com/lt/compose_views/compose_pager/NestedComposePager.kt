 ...
 * effect : 类似于安卓中的ViewPager(支持 nested scroll)
 *          Equivalent to the ViewPager in android(support nested scroll)
 * warning:
        ...
        modifier = modifier
            .nestedScroll(object : NestedScrollConnection {
                override suspend fun onPostFling(
                    ...
                    job?.cancel()
                    job=null
                    if(!a||consumed.x!=available.x) {
                        a=true
                        return super.onPostFling(consumed, available)
                    }
                    val initialVelocity=if(orientation == Orientation.Horizontal) available.x else available.y
                    val index = composePagerState.currSelectIndex.value
                ...
                override fun onPostScroll(
                    ...
                    job=  coroutineScope.launch {
                        scrollableState.scrollBy(available.x)
                    }
